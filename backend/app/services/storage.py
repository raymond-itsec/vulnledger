...
EVIDENCE_BUCKET_NAME = settings.object_storage_evidence_bucket
REPORTS_BUCKET_NAME = settings.object_storage_reports_bucket
...
def ensure_buckets() -> None:
    ...
    for bucket_name, object_lock in (
        (EVIDENCE_BUCKET_NAME, False),
        (REPORTS_BUCKET_NAME, True),
    ):
        if not client.bucket_exists(bucket_name):
            client.make_bucket(bucket_name, object_lock=object_lock)
            logger.info(
                "Created object storage bucket: %s%s",
                bucket_name,
                " with object lock" if object_lock else "",
            )
...
def _put_object(
    ...
    client.put_object(
        ...
        retention=Retention(COMPLIANCE, retain_until) if retain_until else None,
    )
    return storage_key
