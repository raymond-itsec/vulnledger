...
### Changed
- Per-tier compose split. `docker-compose.yml` (335 lines) split into `deploy/compose/{edge,app,data,monitoring}.yml` plus a 5-line root `include:` shim that keeps bare `docker compose up` working. New `deploy/profiles/{allinone,multihost}.env` toggle service-discovery between single-host (Docker DNS) and 4-VM (cross-host) modes. New `deploy/Makefile` exposes `allinone-up / -down / -ps / -build / -config / logs SERVICE=` plus `multihost-up / -down / wg-status` stubs for the upcoming Phases 4-5 work. clamav placed on the data tier (state-bearing-with-state principle). Naming convention captured in inline header comments: `vl-` prefix = VM, `vlan-` prefix = L2 network, `*_net` suffix = Docker compose network.
