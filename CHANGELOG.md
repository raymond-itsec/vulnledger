...
### Changed
- **Breaking (API):** introduced `/api/v1/` URL prefix as the canonical surface. All routers now mount under `/api/v1/` instead of `/api/`. The three direct health endpoints (`/api/health`, `/api/health/live`, `/api/health/debug`) moved to `/api/v1/health/...`. A new `LegacyApiRedirectMiddleware` returns HTTP 308 (preserves method + body, unlike 301) for any unversioned `/api/...` request, redirecting to the `/api/v1/...` equivalent and adding `Deprecation: true` plus `Sunset: Mon, 01 Jun 2026 00:00:00 GMT` headers. The redirect shim exists for transition support of any external API consumer; the frontend SPA is migrated to call `/api/v1/` directly in the same commit. The OpenAPI schema (served at `/openapi.json`) describes v1 routes only; the legacy unversioned paths are an undocumented compatibility shim. Caddy matchers widened to cover both prefixes during the deprecation window: rate-limit exclusions for `/health`, `/health/live`, `/auth/refresh`; tighter `login_brute` bucket on `/auth/login`; and the 30 MB attachment body cap on `/findings/{id}/attachments`. After 2026-06-01, the unversioned shim and the optional `(?:v1/)?` Caddy regex groups can be dropped.
