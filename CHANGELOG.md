...
### Changed
- **API versioning structure:** introduced an `API_VERSIONS` registry in `backend/app/main.py` as the single source of truth for which API versions exist, which is current, and which are deprecated. Routers, OpenAPI URLs, the legacy redirect target, and a new `DeprecatedVersionHeadersMiddleware` all read from the registry rather than hardcoding `/api/v1/`. Adding v2 later is a registry edit (one dict entry) plus the new v2 router modules. The `LegacyApiRedirectMiddleware` now uses a `^/api/v\d+(/|$)` regex to recognize already-versioned paths, so unknown versions like `/api/v999/...` get a clean 404 from FastAPI instead of being treated as unversioned. Middleware order corrected so the legacy 308 short-circuit response still picks up CORS and security headers on the way out (Legacy is innermost, SecurityHeaders outermost; the prior `# outermost first` comment was misleading given Starlette's prepend-on-add semantics).
- **Breaking (API):** introduced `/api/v1/` URL prefix as the canonical surface. All routers now mount under `/api/v1/` instead of `/api/`. The three direct health endpoints (`/api/health`, `/api/health/live`, `/api/health/debug`) moved to `/api/v1/health/...`. The OpenAPI schema, Swagger UI, and ReDoc UI moved from `/openapi.json`, `/docs`, `/redoc` to `/api/v1/openapi.json`, `/api/v1/docs`, `/api/v1/redoc` so they are served under the same Caddy reverse-proxy rule and versioned with the routes. A new `LegacyApiRedirectMiddleware` returns HTTP 308 (preserves method + body, unlike 301) for any unversioned `/api/...` request, redirecting to the `/api/v1/...` equivalent and adding `Deprecation: true` plus `Sunset: Mon, 01 Jun 2026 00:00:00 GMT` headers. The redirect shim exists for transition support of any external API consumer; the frontend SPA is migrated to call `/api/v1/` directly in the same commit. Caddy matchers widened to cover both prefixes during the deprecation window: rate-limit exclusions for `/health`, `/health/live`, `/auth/refresh`; tighter `login_brute` bucket on `/auth/login`; and the 30 MB attachment body cap on `/findings/{id}/attachments`. The backend Docker healthcheck swapped from `/openapi.json` to `/api/v1/health/live` (closes [#25](https://github.com/raymond-itsec/vulnledger/issues/25)). The frontend `generate:types:live` script updated to fetch the schema from the new path. After 2026-06-01, the unversioned shim and the optional `(?:v1/)?` Caddy regex groups can be dropped.
...
### Fixed
- Frontend now schedules a proactive access-token refresh ~60s before the JWT `exp`, instead of only refreshing reactively after a 401. Eliminates the visible 401 in DevTools (and the wasted re-upload of request bodies for file POSTs / DELETEs) every time a session sits idle past the 5-minute access-token TTL. The reactive 401 retry in `client.ts:authorizedFetch` stays in place as a fallback for missed timers (background-tab throttling, large clock skew). Closes [#39](https://github.com/raymond-itsec/vulnledger/issues/39). Touch points: `frontend/src/lib/stores/auth.svelte.ts` adds `decodeTokenExp()`, `scheduleTokenRefresh()`, and `clearTokenRefreshTimer()`, called from every token set/clear site (login, refresh success, bootstrap failure, logout, stale-session cleanup).
...
### Added
- Per-request correlation IDs. Two distinct identifiers, never conflated. `X-Request-ID` is honored on the response only when the incoming request carried a valid UUID under that header (we never invent one ourselves, so a customer's monitoring stack only sees IDs it generated). `X-VL-Request-ID` is always generated server-side as `VL-<uuid4>` and always set on the response; incoming `X-VL-Request-ID` headers are always ignored as the security boundary (only VulnLedger systems get to issue VL-prefixed IDs). Both IDs are exposed via `contextvars.ContextVar` so any code in the request lifecycle can read them, and a new `RequestIDFilter` injects them onto every log record automatically. Multi-host (Phase 5+) plan: VL services forward `X-VL-Request-ID` to other VL services for trustworthy internal trace correlation; `X-Request-ID` propagates client-facing for monitoring-stack continuity. Implementation: `backend/app/middleware/request_id.py` (new), wired into `main.py` as the outermost middleware, with logging config updated in `backend/app/logging_config.py`.

### Changed
- Alembic and SQLAlchemy log lines now go through the application's JSON formatter instead of alembic.ini's bare-text `[formatter_generic]`. `backend/alembic/env.py` calls `app.logging_config.configure_logging()` directly and no longer invokes `fileConfig()`; `backend/alembic.ini` had its `[loggers]` / `[handlers]` / `[formatters]` blocks removed so the file is just the alembic-script config now. One log shape across the whole stack (uvicorn, app, alembic, sqlalchemy). Logs emitted during migrations naturally show `x_request_id` and `vl_request_id` as null because no HTTP request is in flight.

- **Breaking (API):** standardized error response shape across every error path. Errors now return a canonical envelope `{"error": {"code", "message", "request_id", "x_request_id" (when upstream provided), "details" (when structured), "timestamp"}}` instead of the prior flat `{"code", "detail", "timestamp"}` shape. The `code` field stays a stable snake_case string the frontend can switch on; `message` is human-readable (display, do not parse); `request_id` carries the `vl_request_id` from Phase 1.5 so users can paste it in a bug report and you grep one log line. `x_request_id` is omitted when no valid upstream UUID was received; `details` is omitted when no structured payload exists. Validation errors now populate `details.errors` with Pydantic's per-field error list so frontends can highlight the offending fields without parsing free-form text. The four global exception handlers (rate-limit, HTTPException, validation, unhandled) all use the new envelope; per-route `HTTPException` raises flow through the global `HTTPException` handler so they inherit it too. Frontend `readPublicErrorMessage` updated to read the canonical envelope (with legacy flat-shape fallback for the deprecation window) and append `request_id` to the returned string so every toast surfaces it automatically.
