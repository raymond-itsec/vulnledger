...
### Changed
- Per-tier compose split. `docker-compose.yml` (335 lines) split into `deploy/compose/{edge,app,data,monitoring}.yml` plus a 5-line root `include:` shim that keeps bare `docker compose up` working. `deploy/profiles/allinone.env` for single-host (Docker DNS service discovery); `deploy/profiles/multihost-samedc.env` and `multihost-crossdc.env` for the two multi-host shapes. `deploy/Makefile` exposes `allinone-up / -down / -ps / -build / -config / logs SERVICE=`, `multihost-samedc-{up,down}`, `multihost-crossdc-{up,down}`, plus `wg-keys / wg-render / wg-bootstrap / wg-install / wg-status` for the WireGuard mesh helper. clamav placed on the data tier (state-bearing-with-state principle). Naming convention captured in inline header comments: `vl-` prefix = host, `*_net` suffix = Docker compose network.
- Frontend image bumped from Node 22.22-alpine to Node 24.15-alpine (LTS). Node 24 entered Active LTS 2025-10-28; Node 22 stays in Maintenance LTS until 2027-04-30, so this is a no-rush move onto the current LTS line. Lockfile regenerated; 53/53 vitest pass; static-adapter build clean. Closes [PR #56](https://github.com/raymond-itsec/vulnledger/pull/56).
- Single source of truth for Node major version via `.nvmrc` at the repo root. All four CI workflows read it via `setup-node`'s `node-version-file`. The frontend `Dockerfile` keeps its specific minor + digest pin (supply-chain hardening) but carries an inline note that the major must stay in sync with `.nvmrc`, plus a 3-step bump procedure.
- Backend image bumped from `python:3.12.13-slim-bookworm` to `python:3.14.5-slim-bookworm`. The `.python-version` dev marker and the `python-version` pin in all five CI workflows (backend-tests, lockfile-sync, security-audit, semgrep, docs) move in lockstep so runtime, local dev, and CI agree. `requirements.lock.txt` regenerated under Python 3.14; the backend image builds clean with every C-extension dependency resolving a cp314 wheel. Closes [PR #124](https://github.com/raymond-itsec/vulnledger/pull/124).
- Multi-host deployment model rewritten as hardware-agnostic: three shapes (allinone / same-DC / cross-DC), a documented connectivity contract, and EU-friendly provider examples (OVH, Hetzner, Scaleway, Contabo, bare metal, Proxmox, ESXi). The compose tier files and connectivity contract don't depend on any specific hypervisor or provider. The optional ESXi reference recipe lives in a separate companion repo (see Added).
- Documentation refresh: `docs/api.md` flipped to canonical `/api/v1/...` paths with deprecation note for the 308 legacy redirect; `docs/architecture.md` ASCII diagram, request flow, auth + OIDC URLs, and project-structure tree updated to reflect the post-compose-split layout; `docs/security.md` rate-limit and body-cap URLs versioned.
- Centralized `COOKIE_SECURE` derivation. `Settings.cookie_secure` property in `backend/app/config.py` is the single source of truth (was previously re-derived identically in `app/api/auth.py`, `app/api/oidc.py`, `app/api/onboarding.py`). Pure code hygiene; no behavior change. Closes [#22](https://github.com/raymond-itsec/vulnledger/issues/22).
- Harmonized config defaults across `backend/app/config.py`, `deploy/compose/*.yml`, and `.env.example`. Added missing entries to `.env.example` (`FINDINGS_OBJECT_STORAGE_ENDPOINT`, `FINDINGS_CLAMAV_HOST`, `FINDINGS_CLAMAV_PORT`, `BACKUP_RETENTION_DAYS`, `BACKUP_CRON`). Forwarded `FINDINGS_SWAGGER_UI_ENABLED` and `FINDINGS_REDOC_ENABLED` through the backend container env so operator overrides via `.env` actually take effect (footgun fix). Parameterized backup container `BACKUP_RETENTION_DAYS` and `BACKUP_CRON` (were hardcoded). Inline comments document the three intentional drifts that remain (compose `findings` default vs `.env.example` placeholder for `POSTGRES_USER`/`DB`; `config.py` empty default for `jwt_*_file` vs `.env.example` populated path; `config.py` `app_version="0.0.0"` safety fallback). Closes [#37](https://github.com/raymond-itsec/vulnledger/issues/37).
- Caddy is now the single source of truth for security response headers (X-Content-Type-Options, X-Frame-Options, Referrer-Policy, Permissions-Policy, Content-Security-Policy, Cross-Origin-Opener-Policy, Cross-Origin-Resource-Policy, X-Permitted-Cross-Domain-Policies, Strict-Transport-Security). The backend used to set its own copies, which produced duplicate response headers (sometimes with conflicting values, e.g. our Permissions-Policy needs `clipboard-write=(self)` for the copy buttons but the backend version omitted it). Backend's `SecurityHeadersMiddleware` renamed to `CacheControlMiddleware` and stripped down to the one app-aware directive that should travel with the app code: `Cache-Control: no-store` on `/api/*` responses. Also dropped `X-XSS-Protection` entirely (deprecated and ignored by every modern browser). Two new backend tests lock the contract.
- Introduced `/api/v1/` URL prefix as the canonical surface; unversioned `/api/...` requests return HTTP 308 to `/api/v1/...` with `Deprecation: true` and `Sunset: Mon, 01 Jun 2026 00:00:00 GMT` headers.
- Moved OpenAPI schema, Swagger UI, and ReDoc UI from `/openapi.json`, `/docs`, `/redoc` to `/api/v1/openapi.json`, `/api/v1/docs`, `/api/v1/redoc`.
- Added `API_VERSIONS` registry as the single source of truth for which versions exist, which is current, and which are deprecated; routers, OpenAPI URLs, and middleware all derive from it.
- Added `DeprecatedVersionHeadersMiddleware` (no-op until a version is deprecated) and corrected middleware order so the 308 short-circuit response still picks up CORS and security headers.
- Standardized pagination shape across five list endpoints (`/findings/{id}/attachments`, `/taxonomy/versions`, `/users/reviewers`, `/auth/sessions`, `/auth/security-events`) to the canonical `{items, total, page, per_page, pages}` envelope; frontend updated in lockstep.
- Renamed `auth/security-events` `limit` query param to `per_page` (same semantics, ge=1, le=200).
- Removed obsolete `SessionListResponse` and `SecurityEventListResponse` Pydantic schemas.
- Standardized error response shape across every error path: `{"error": {"code", "message", "request_id", "x_request_id" (when populated), "details" (when populated), "timestamp"}}` replaces the prior flat `{"code", "detail", "timestamp"}` shape.
- Validation errors populate `details.errors` with Pydantic per-field issues so frontends can highlight offending fields.
- Frontend `readPublicErrorMessage` reads the canonical envelope and appends `(Error ID: VL-...)` to every toast so users can paste the ID in bug reports.
- Backend Docker healthcheck swapped from `/openapi.json` to `/api/v1/health/live`. Closes [#25](https://github.com/raymond-itsec/vulnledger/issues/25).
- Caddy matchers widened with optional `(?:v1/)?` to cover both prefixes during the deprecation window (attachments body cap, rate-limit exclusions, login brute bucket).
- Routed alembic and SQLAlchemy logs through the application's JSON formatter; `alembic.ini` `[loggers]` / `[handlers]` / `[formatters]` blocks removed.

### Added
- WireGuard mesh automation for cross-DC multi-host deployments. `deploy/wireguard/peers.example.txt` defines per-peer name + WG overlay IP + public endpoint; `generate-keys.sh` produces keypairs (gitignored); `render-configs.sh` writes complete `/etc/wireguard/wg0.conf` per peer (gitignored); `install-configs.sh` scps each rendered conf to its peer over SSH and enables `wg-quick@wg0` via systemd. `make wg-bootstrap` chains keys + render in one step; `make wg-install` covers the SSH delivery. Cold-bootstrap path (cloud-init injection of the rendered conf) called out for the case where SSH is not yet reachable. Reduces mesh setup from per-peer hand-edits of a template to ~3 commands.
- Lockfile auto-fix workflow (`.github/workflows/lockfile-sync.yml`). Detects drift between `package.json`/`requirements.txt` and their lockfiles on every push and PR; regenerates the lockfile and commits it back as `github-actions[bot]`. Uses a fine-grained `LOCKFILE_PAT` repo secret so the auto-commit retriggers downstream CI (the default `GITHUB_TOKEN` does not). Eliminates the recurring "Missing: X from lock file" CI failure when a dependency is bumped without the lockfile regenerated in lockstep.
- `vulnledger-lab-esxi` companion repo at [github.com/raymond-itsec/vulnledger-lab-esxi](https://github.com/raymond-itsec/vulnledger-lab-esxi) holds the optional reference recipe for running the 4-tier multihost on VMware ESXi free tier with Rocky Linux 10.1 (Packer template, kickstart, cloud-init, ovftool deploy script, nftables rulesets). Explicitly NOT a prerequisite — VulnLedger runs on bare metal, KVM, Proxmox, ESXi, or any provider's VPS that runs Docker on Linux.
- Wishlist + call for testers in `README.md`. Six community-actionable ideas in priority order: scanner-import plugins (Burp, ZAP, Nessus, Nuclei, Nmap, Trivy, Semgrep, CodeQL, generic SARIF) with sample exports welcome at <support@vulnledger.app>; export connectors to Jira / GitLab / GitHub / Mattermost / Rocket.Chat; SBOM ingestion (CycloneDX, SPDX); branded PDF reports; webhook outputs; localization (Dutch / German / French to start).
- Typed `ApiError` class on the frontend wraps the canonical envelope; new `<FieldError>`, `<ErrorState>`, and `<ErrorBoundary>` design-system components. All forms (login, onboarding, invite redeem, client / asset / session / finding / template create + edit, profile, admin invite issue) now route 422 validation errors inline next to the offending input via `extractFieldErrors`; non-validation errors fall through to a toast that includes the `(Error ID: VL-...)` suffix. App-level `<ErrorBoundary>` in the root layout catches uncaught render errors so a broken subtree no longer white-screens. New `vitest` test framework (39 unit + component tests covering `ApiError`, `FieldError`, `ErrorState`, `ErrorBoundary`, `handleFormError`) and `frontend-tests.yml` GitHub workflow.
- Prometheus metrics endpoint at `/api/v1/metrics`. Restricted to RFC1918 + loopback source IPs (same pattern as the liveness probe). Emits HTTP request counters / latency histograms / in-flight gauges (per method + route template + status), per-error-code counts (from the standardized error envelope), DB pool size / checked-out / checked-in, app-info marker, and 8 business gauges (clients, assets, sessions, findings by risk level, users by role and active state, pending invites, attachments, report exports by format). Bare `prometheus-client==0.21.1`; ~60 lines of middleware. Single-process mode for now (single uvicorn worker per container); switch to `prometheus_client.multiprocess` when scaling out workers. Caddy rate-limit exclusion added so vmagent scraping does not throttle. Part of [#27](https://github.com/raymond-itsec/vulnledger/issues/27).
- Observability stack initial bring-up ([#27](https://github.com/raymond-itsec/vulnledger/issues/27)). New `monitoring/` directory ships configs for VictoriaMetrics + vmagent + vmalert (TSDB, scrape, rule evaluator), Alertmanager (Slack default route + Watchdog deadman heartbeat receiver), Grafana with auto-provisioned datasources, Loki + Promtail with JSON pipeline-stage parsing of backend's structured logs, node-exporter, cAdvisor, and postgres-exporter. New `deploy/compose/monitoring.yml` runs the central tier; per-tier sidecars on `edge.yml` / `app.yml` / `data.yml` ship local metrics and logs (per-host vmagent so each tier buffers across cross-DC link blips, capped at 1GB on-disk via `--remoteWrite.maxDiskUsagePerURL` so a long monitoring outage never fills host disk). Backend exposes `vl_clamav_up` (refreshed via `probe_scanner()` on each scrape). Backup container writes `vl_backup_latest_timestamp_seconds` to a textfile-collector volume shared with node-exporter on the data tier; `BackupTooOld` alert derives age via PromQL `time() - <metric>`. Image versions hoisted to `deploy/profiles/versions.env` so the VM trio (vmagent / vmalert / VictoriaMetrics) and the Loki / Promtail pair share one variable each and can't drift across tiers; image refs use `${VERSION}${IMAGE_DIGEST:-}` so digest pins land via env without re-touching compose. Caddyfile adds `/grafana/*` reverse proxy (allinone) and exposes `:2019/metrics` for vmagent scraping. Initial scope: 4 alert rules (BackupTooOld + BackupMetricMissing, ContainerCrashLooping, ClamAVDown, Watchdog deadman). Digest pinning of every image to `@sha256:...` tracked at [#79](https://github.com/raymond-itsec/vulnledger/issues/79).
- Observability stack [#27](https://github.com/raymond-itsec/vulnledger/issues/27) completion: the alerting and synthetic checks the initial bring-up deferred. A synthetic auth-flow probe (a Python `/metrics` service on the app tier that runs login, refresh, and logout against the backend every `PROBE_INTERVAL` seconds, exposing `vl_synthetic_auth_*`) and a blackbox-exporter frontend smoke check (GET / against the frontend, asserting the `VulnLedger` branding token). Four vmalert additions: `disk.rules.yml` (root filesystem over 80% / 95% full), `ratelimit.rules.yml` (sustained HTTP 429 rate), `probe.rules.yml` (synthetic-auth and frontend-smoke alerts plus a TLS-expiry rule that activates once HTTPS fronts prod), and a `BackupNotEncrypted` posture alert backed by two new `vl_backup_latest_encrypted` / `vl_backup_encryption_expected` metrics written by `backup.sh`. Both new services run in allinone, samedc, and crossdc with no per-mode change: the probe sits on the app tier next to the backend, blackbox next to the frontend, each scraped by its tier's local vmagent. Loki log retention raised from 30 days to 1 year to match the VictoriaMetrics metric-retention window (size the `loki_data` volume accordingly). A committed Grafana dashboard `vulnledger-overview.json` auto-loads via the existing provisioner. The CI `frontend-openapi-types` job now fails on uncommitted `openapi.generated.json` / `generated.ts` drift instead of silently regenerating. A low-privilege synthetic monitoring user is optionally seeded from `FINDINGS_SYNTHETIC_USER_*`.
- `FINDINGS_SWAGGER_UI_ENABLED` and `FINDINGS_REDOC_ENABLED` env flags to toggle the interactive API docs (both default ON; OpenAPI JSON always served).
- Per-request correlation IDs. `X-Request-ID` honored on response only when incoming carries a valid UUID (never invented by us). `X-VL-Request-ID` always generated server-side as `VL-<uuid4>`; incoming variant always ignored as the security boundary. Both IDs injected into every log record automatically.
- New `backend/app/middleware/request_id.py`.

### Fixed
- VL-2026-017 ([#41](https://github.com/raymond-itsec/vulnledger/issues/41)): invite redemption now serializes at the invite row via `SELECT ... FOR UPDATE`. Previously two concurrent redemptions of the same invite both passed the `claimed_at IS NULL` check and raced into a downstream `users.email` UNIQUE-constraint collision; the redemption was effectively single-use only by accident, through an invariant that wasn't the right holder. The fix opts the redemption path into a row lock via a new `lock=True` parameter on `_resolve_active_invite`; the read-only state endpoint stays unlocked so parallel state polls from other tabs don't block. Backend test added: two `asyncio.gather`'d redemptions assert exactly one wins and one rejects with 401, and exactly one user row is created.
- Reliability: `create_taxonomy_version` ([#48](https://github.com/raymond-itsec/vulnledger/issues/48)) was vulnerable to a race when two admins created versions concurrently - both could compute the same `max(version_number) + 1` and the loser would surface an unhandled `IntegrityError` as HTTP 500. Service now wraps the version-row insert in a bounded retry (3 attempts) that catches `IntegrityError`, rolls back, and re-reads the max; if all attempts collide, raises a new `TaxonomyConflictError` that the API translates to 409. Backend test added to lock the contract via two concurrent `asyncio.gather` calls.
- Reliability ([#76](https://github.com/raymond-itsec/vulnledger/issues/76)): boot-time seeding paths (`seed_admin_user`, `sync_builtin_templates`, `ensure_default_taxonomy_version`) now catch `IntegrityError` on commit and treat it as success. Previously, two backend containers booting simultaneously could both pass the existence check and both try to insert; the loser crash-looped on the unique-constraint violation until restart. Defensive cleanup; matters more once the app tier scales horizontally per [#38](https://github.com/raymond-itsec/vulnledger/issues/38).
- Reliability: OIDC auto-provision callback ([#47](https://github.com/raymond-itsec/vulnledger/issues/47)) was vulnerable to a race when two callbacks for the same first-time `(issuer, subject)` arrived concurrently - both passed the initial lookup and tried to insert; the unique-constraint loser surfaced an unhandled `IntegrityError` as HTTP 500. Now catches `IntegrityError`, rolls back, re-reads by `(issuer, subject)`, and returns the winner's user. If the post-rollback re-read finds no winner (a different unique-constraint collision, e.g. username/email conflict with an unrelated account), surfaces a controlled 409 instead of a generic 500. Backend test added.
- VL-2026-020 ([#61](https://github.com/raymond-itsec/vulnledger/issues/61)): onboarding form used HTML5 `required` and `minlength` attributes, so the browser's native validation popup fired on submit and short-circuited the form's `onsubmit` handler. Backend never got called, the unified `<FieldError>` component never rendered, and users saw a browser popup instead of inline styled errors with copyable request IDs. The `minlength="12"` hint also lied about the actual policy (`FINDINGS_PASSWORD_MIN_LENGTH` defaults to 16). Added `novalidate` to the form so all errors route through the unified system; dropped the misleading `minlength` hints to keep backend as the single source of truth for password policy. Spot-check confirmed onboarding was the only form in the app with this pattern.
- VL-2026-019 ([#60](https://github.com/raymond-itsec/vulnledger/issues/60)): `POST /api/v1/invites/verify` returned HTTP 404 when the invite code was invalid / claimed / revoked / expired. The URL exists; what failed is credential validation, so 401 is the right status. The single uniform message ("invalid or expired") still covers all four failure cases so attackers cannot enumerate valid codes by classifying responses. Brings the endpoint in line with the existing 401 in the same module for "no token presented".
- VL-2026-018 ([#59](https://github.com/raymond-itsec/vulnledger/issues/59)): onboarding cookie path not migrated alongside the `/api/v1/` URL prefix. Cookie was scoped to `/api/onboarding` but the routes mount under `/api/v1/onboarding/*` via the `API_VERSIONS` registry. Per RFC 6265 path matching, browsers never sent the cookie on `/api/v1/onboarding/state` or `/api/v1/onboarding/complete`, so every invited-user signup flow returned 401 after the verify step. Same bug class as the refresh-token cookie regression already fixed for `auth.py`; one file (`onboarding.py`) was missed during that sweep. Cookie paths now derive from `app.versioning.CURRENT_API_PREFIX` (consistent with `auth.py` and `oidc.py`); `_clear_onboarding_cookie` deletes at both the new and legacy paths so phantom cookies in users' jars get cleaned up. Backend test added to lock the contract.
- Refresh-token and OIDC cookie paths were not migrated alongside the `/api/v1/` URL prefix. The cookies were scoped to `/api/auth` and `/api/auth/oidc`, but the routes had moved to `/api/v1/auth/...`. Per RFC 6265 path matching, browsers stopped sending the cookies on `/api/v1/auth/refresh`, causing every refresh to 401 ~5 minutes after login (the access-token TTL) and forcing repeated logouts. Cookie paths are now derived from a new `app.versioning` module so a future v2 cutover updates both router mounts and cookie paths in lockstep, with a startup sanity check that fails loudly on drift. Logout / failure responses also clear the legacy cookie at `/api/auth` so users with stale cookies in their browser jar from before this fix get cleaned up automatically.
- SeaweedFS `volume.max` bumped from default 8 to 100 in `docker-compose.yml`. Silent attachment upload failures since the storage migration with `No writable volumes and no free volumes left for collection:finding-attachments`.
- Frontend schedules a proactive access-token refresh ~60s before JWT `exp` instead of refreshing reactively after a 401. Eliminates wasted re-upload of file bodies on idle sessions. Reactive 401 retry stays as fallback. Closes [#39](https://github.com/raymond-itsec/vulnledger/issues/39).
- Replaced two `datetime.utcnow()` calls in `backend/app/services/reports.py` with `datetime.now(timezone.utc)` (deprecated in Python 3.12; lost UTC marker on JSON serialization).

### Security
- Caddy now strips the upstream `Server` header (which exposed `uvicorn` to anyone running `curl -I`) and replaces it with a generic `Server: VulnLedger`. Closes [#58](https://github.com/raymond-itsec/vulnledger/issues/58). Implementation lives inside the `reverse_proxy` block as `header_down Server "VulnLedger"` after a chain of attempts at the site-block level kept being overwritten by Caddy's response writer on the way out (documented in code comment). Hardening is low-impact (the version string was the only signal exposed) but free.
- Bumped `urllib3` 2.6.3 to 2.7.0, fixing two high-severity advisories: GHSA-mf9v-mfxr-j63j (decompression-bomb safeguard bypass in the streaming API) and GHSA-qccp-gfcp-xxvc (sensitive headers not stripped on cross-host redirect via `ProxyManager`). urllib3 is a transitive dependency via `minio` and `requests`, so only `backend/requirements.lock.txt` changed. Landed in [#119](https://github.com/raymond-itsec/vulnledger/pull/119), which supersedes Dependabot PRs [#103](https://github.com/raymond-itsec/vulnledger/pull/103) and [#82](https://github.com/raymond-itsec/vulnledger/pull/82).
- Patched three high-severity OS-package CVEs in the backend Docker image: CVE-2026-0861 (glibc heap corruption via integer overflow in `memalign`), CVE-2026-4878 (libcap2 privilege escalation through a TOCTOU race in `cap_set_file()`), and CVE-2026-29111 (systemd code execution via spurious IPC). The `python:3.12.13-slim-bookworm` base image lagged the published Debian fixes, so `apt-get upgrade` was added to the backend Dockerfile build step to pull patched packages at build time. Landed in [#119](https://github.com/raymond-itsec/vulnledger/pull/119).
- Bumped `fast-uri` 3.1.1 to 3.1.2, fixing advisory GHSA-v39h-62p7-jpjc. fast-uri is a transitive frontend dependency, so only the npm lockfile changed. Landed in [#99](https://github.com/raymond-itsec/vulnledger/pull/99).
- Bumped `idna` 3.13 to 3.15, picking up the CVE-2026-45409 fix that strengthens the earlier CVE-2024-3651 mitigation. Landed in [#108](https://github.com/raymond-itsec/vulnledger/pull/108).

### Audited (no code change required)
- httpx: zero direct uses in application code; pinned only as a transitive dependency for Authlib's OIDC discovery + token exchange.
