...
- Reliability: boot-time seeding paths (`seed_admin_user`, `sync_builtin_templates`, `ensure_default_taxonomy_version`) now catch `IntegrityError` on commit and treat it as success. Previously, two backend containers booting simultaneously could both pass the existence check and both try to insert; the loser crash-looped on the unique-constraint violation until restart. Defensive cleanup; matters more once the app tier scales horizontally.
